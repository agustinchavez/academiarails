...
describe User do
  ...
  describe "expire" do
    ...
    it "sends an email to user" do
      expect { @user.expire }.to change(ExpirationEmailWorker.jobs, :size).by(1)
    end
  ...
  describe "saving a User" do
    ...
    it "sends details to Stripe" do
      @user = User.new(email: "test@testing.com",
        stripe_token: 12345, name: 'tester', password: 'password')
      expect { @user.save }.to change(CreateCustomerWorker.jobs, :size).by(1)
    end
  ...
  describe "getting data from the Stripe API" do
    ...
    it "keeps track of the last 4 digits in the credit card" do
