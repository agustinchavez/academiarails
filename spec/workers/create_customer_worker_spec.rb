...
describe CreateCustomerWorker do
  ...
  it "uses a User record to create a customer on Stripe's end" do
    @create_customer_worker.user = @user
    StripeCustomer.should_receive(:create).and_return(@customer)
    @create_customer_worker.create_customer("12345", "DISCOUNT_CODE")
  end
